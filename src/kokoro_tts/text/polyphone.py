"""Model-aware lightweight Chinese polyphone rules."""
...
def _uses_kokoro_polyphone_proxy(model: str) -> bool:
    raw = str(model or "kokoro").strip().lower()
    return not (raw.startswith("moss") or raw.startswith("zipvoice"))
...
def apply_polyphone_overrides(text: str, model: str = "kokoro") -> str:
    ...
    # 选择模型对应的规则集。
    uses_proxy = _uses_kokoro_polyphone_proxy(model)
    phrases = _POLYPHONE_PHRASES_KOKORO if uses_proxy else _POLYPHONE_PHRASES_MOSS
    liao_patterns = _LIAO_PATTERNS_KOKORO if uses_proxy else _LIAO_PATTERNS_MOSS
