...
| MOSS 运行时优化 | 单 engine executor + runtime lock，参考音频裁剪与 prompt code LRU 缓存，输出削峰归一化，降低 clone OOM、爆音和重复编码开销 |
...
MOSS 克隆参考音频默认裁剪到 10 秒，并缓存编码后的 prompt audio codes。WebSocket 克隆是 AngeVoice 层面的逐段流式：首包上传参考音频，后端按文本分片生成并推送音频；它不是上游模型 token 级真实流式。
