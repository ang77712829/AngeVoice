...
v2.6 完成服务端模块化重构、中文规则补强、Studio Web UI 刷新、安全启动校验和多模型运行时，并统一项目品牌为 AngeVoice。批量合成、管理接口、可选 MP3、WebSocket 取消能力保持兼容。
...
| MOSS 运行时优化 | 单 engine executor + runtime lock，官方 `generate_audio_frames` 回调增量 decode，参考音频裁剪与 prompt code LRU 缓存，输出削峰归一化，降低 clone OOM、爆音和重复编码开销 |
...
MOSS 克隆参考音频默认裁剪到 10 秒，并缓存编码后的 prompt audio codes。WebSocket 克隆会接入 OpenMOSS 官方 `generate_audio_frames` 回调，在 codec frame 生成过程中增量 decode 并推送小音频包；它不是 token 级流式，但不会再等待整段 MOSS chunk 完成后才推送。
