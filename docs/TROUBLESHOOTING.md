...
如果 `moss-nano-cpu` / `moss-nano-cuda` 显示 `available=false`，通常是官方 OpenMOSS runtime 没有安装、`MOSS_TTS_NANO_PATH` 指向错误，或当前镜像不是 v2.6 之后的 MOSS 预装画像。先确认正在使用最新 Dockerfile/Compose，并检查：
...
如果 `last_output_quality.max_abs_before` 经常接近或超过 `1.0`，说明上游波形已经接近削波，先不要提高 `MOSS_OUTPUT_GAIN`。如果 clone 一直 OOM，继续缩短 `MOSS_PROMPT_AUDIO_MAX_SECONDS` 到 `5`，或先切换 `moss-nano-cpu` 验证文本和参考音频本身。
...
单张 8GB 卡不建议同时跑多个会加载 GPU 模型的容器。`MOSS_CUDA_MEMORY_LIMIT_MB` 默认保持 `0`，不会限制通用 GPU；只有在 Tesla P4、RTX 3070 这类紧张环境排障时，才建议手动设置成 `4096` 或更低测试。

如果 WebSocket 长文本日志里看到单个 MOSS chunk 一次吐出几十万到上百万 samples，并伴随 `keepalive ping failed`，说明运行的是旧版 adapter。新版会打印 `MOSS runtime chunk ... streamed`，并持续发送小音频包。
...
WebSocket 克隆也可用，但参考音频需要放在首个 JSON 的 `prompt_audio.data` 字段中。Studio Web UI 会自动处理；自写客户端可参考：
